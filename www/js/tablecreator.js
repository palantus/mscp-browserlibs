...
TableCreator.prototype.draw = function(callback){
	...
	if(this.options.hideHeader !== true){
		...
		if(t.options.showPrint !== false && !window.isMobile()){
			var printButton = $("<button title='Print'><img style='width:15px;height:12px' src='/img/print.png'></img></button>");
		...
		if(this.options.dataSource !== undefined){
			var refreshButton = $("<button title='Refresh'><img style='width:15px;height:12px' src='/img/refresh.png'></img></button>");
...
TableCreator.prototype.refreshData = function(_startIdx, _endIdx){
	...
	*/
	...
	for(var r = startIdx; r <= endIdx && !this.isLoading; r++){
		...
		if(typeof(this.options.editRecord) === "object"){
			var cell = $("<td class='tceditcell'><img src='/img/edit.ico' title='" + this.label("editrecord") + "'></img></td>");
		...
		if(typeof(this.options.deleteRecord) === "object"){
			var cell = $("<td class='tcdeletecell'><img src='/img/delete.png' title='" + this.label("deleterow") + "'></img></td>");
			cell.data("tcdatarecord", this.data[r]);
			var t = this;
