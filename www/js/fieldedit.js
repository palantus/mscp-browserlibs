...
FieldEdit.prototype.init = function(_options){
  ...
  if(typeof(this.options.values) === "function"){
    let vals = this.options.values()

    if(Promise.resolve(vals) == vals){
      this.valuesPromise = vals;
    } else {
      t.options.values = vals;
    }
  } else if(Promise.resolve(this.options.values) == this.options.values){
    this.valuesPromise = this.options.values;
  }
...
FieldEdit.prototype.show = async function(setFocus){
  var originalWidth = this.element.width();
  var originalHeight = this.element.height();

  if(this.valuesPromise != null)
    this.options.values = await this.valuesPromise;
